...
type GeoJsonFeature<
  G extends GeoJsonPoint | GeoJsonLineString,
...
const MapComponent = forwardRef<MapRef, MapProps>(function Map(
  {
    children,
    className,
    theme: themeProp,
  ...
  // Keep the canvas in sync with responsive dashboard layouts.
  // Without this, MapLibre can keep the initial canvas height and let the
  // parent background show through as a strip at the bottom.
  useEffect(() => {
...
type MapControlsProps = {
  ...
  showLocate?: boolean;
  /** Disable the locate button while keeping it visible (default: false) */
  locateDisabled?: boolean;
  /** Accessible label/title used when locate is disabled */
  locateDisabledLabel?: string;
...
function MapControls({
  ...
  locateDisabledLabel = "Find my location disabled",
      ...
      {showLocate && (
        <ControlGroup>
          <ControlButton
            ...
            label={locateDisabled ? locateDisabledLabel : "Find my location"}
            title={locateDisabled ? locateDisabledLabel : "Find my location"}
            disabled={locateDisabled || waitingForLocation}
          >
            {waitingForLocation ? (
...
function MapArc<T extends MapArcDatum = MapArcDatum>({
  ...
  const geoJSON = useMemo<GeoJsonFeatureCollection<GeoJsonLineString>>(
    () => ({
      type: "FeatureCollection",
